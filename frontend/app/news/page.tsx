...
const ENTRIES: Entry[] = [
  {
    date: "2026 · 05 · 01",
    kind: "dispatch",
    title: "DocSouth · BTUT + ripser H2 + RunPod GPU on a real archive",
    blurb:
      "First private model formed on a real institutional corpus and exercised through every async path the substrate exposes. Documenting the American South, UNC Libraries' digitized archive of Southern history — 37,505 chapter-sized segments across the four DocSouth collections (Slave Narratives, First-Person Narratives, Library of Southern Literature, Church in the Southern Black Community), 65 MB plain text from 711 source texts. Formation: 121.8 seconds end-to-end. (1) BTUT primary path — real Python pipeline via HTTP bridge, 8 chunks of 5,000 records each merged in 45 s, 2,400 representative survivor fingerprints, fingerprinter_mode=btut, effective_strategy=btut. Sparse fallback (MinHash · SimHash · Bloom · byte-hash) correctly stayed dormant — primary covered. (2) Persistent homology via ripser — Vietoris-Rips over the survivors with Hamming-48 metric, threshold 24, computed in the api container in 74.5 s on 600 stride-sampled points. Resulting bars: H0 = 522 connected components, H1 = 179 cycles, H2 = 77 multi-dimensional voids — 778 bars total, all real. (3) RunPod GPU async — cuda device, 50 epochs, 3 discovered modules, final_auc 0.9045, completed 65 s after submit. (4) Determinism contract: 7/7 query intents byte-identical on re-issue. (5) Multi-tenant isolation: cross-tenant probe returned 404, self returned 200. (6) Audit log: events captured in JSON / CEF / OCSF formats. Artifact encrypted at rest with AES-256-GCM. Tenant: docsouth_showcase. Model: rng_772c6fb01bea38cb6a3a.",
