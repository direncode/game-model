...
import freeArtifactRaw from "@/data/showcases/universal/substrate_artifact.json";
import premiumArtifactRaw from "@/data/showcases/universal/substrate_artifact_pro.json";
...
const sources = (() => {
  try {
    const text = readFileSync(join(STDLIB, "universal.ocean"), "utf-8");
    ...
    <>
      ...
      <main className="max-w-6xl mx-auto px-6 pt-32 pb-16">
        <ShowcaseHeader
          namespace="universal"
          dataset="substrate"
          title="Universal: Cross-Domain Substrate"
          tagline="Six source types in one structural-clustering pass."
          labelField="source_type"
          free={freeArtifact}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
          <ShowcaseArtifactPanel artifact={freeArtifact} tier="free" labelField="source_type" />
          <ShowcaseArtifactPanel artifact={premiumArtifact} tier="premium" labelField="source_type" />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <ShowcaseSourcePanel source={sources.free} presetName="universal.substrate" tier="free" />
          <ShowcaseSourcePanel source={sources.premium} presetName="universal.substrate_pro" tier="premium" />
        </div>
        <ShowcaseFooter presetName="universal.substrate" />
      </main>
    </>
  );
}
