#!/bin/bash
# Atlas post-formation verification.
#
# After /api/range-form completes for the atlas_showcase tenant, this:
#   1. lists models for atlas_showcase tenant
...
#
# Requires:  /tmp/.atlastoken (atlas_showcase bearer)
# Output:    /tmp/atlas_summary.json

set -e
BASE="${LO_BASE_URL:-https://www.latentocean.com}"
ATLASTOK=$(cat /tmp/.atlastoken)

# 1. List models
echo "=== 1. List models for atlas_showcase tenant ==="
LIST=$(curl -sk -H "Authorization: Bearer $ATLASTOK" "$BASE/api/range-form")
MODEL_ID=$(echo "$LIST" | python -c "import sys,json;d=json.load(sys.stdin);ms=d.get('models',[]);print([m for m in ms if 'Atlas' in m['name'] or 'arxiv' in m['name'].lower()][0]['id'] if any('Atlas' in m['name'] or 'arxiv' in m['name'].lower() for m in ms) else (ms[0]['id'] if ms else ''))" 2>/dev/null)
if [ -z "$MODEL_ID" ]; then
...
META=$(curl -sk -H "Authorization: Bearer $ATLASTOK" "$BASE/api/range-form/$MODEL_ID")
echo "$META" | python -c "
import sys, json
...
for intent in what_is_rare novel_classes show_taxonomy describe_corpus compare_to_llm summarize lineage_trace; do
  Q="${INTENTS[$intent]}"
  RESP=$(curl -sk -H "Authorization: Bearer $ATLASTOK" "$BASE/api/range-query?model_id=$MODEL_ID&q=$(python -c "import urllib.parse,sys;print(urllib.parse.quote(sys.argv[1]))" "$Q")")
...
echo "  GET atlas_showcase model with probe token: $PROBE_STATUS (expect 404)"
SELF_STATUS=$(curl -sk -o /dev/null -w '%{http_code}' -H "Authorization: Bearer $ATLASTOK" "$BASE/api/range-form/$MODEL_ID")
echo "  GET atlas_showcase model with own token:   $SELF_STATUS (expect 200)"

# 6. Pull audit
echo ""
echo "=== 6. Audit log ==="
JSON_AUDIT=$(curl -sk -H "Authorization: Bearer $ATLASTOK" "$BASE/api/range-audit?limit=20")
...
CEF_LINES=$(curl -sk -H "Authorization: Bearer $ATLASTOK" "$BASE/api/range-audit?format=cef&limit=20" | wc -l)
echo "  CEF: $CEF_LINES lines"
OCSF_COUNT=$(curl -sk -H "Authorization: Bearer $ATLASTOK" "$BASE/api/range-audit?format=ocsf&limit=20" | python -c "import sys,json;d=json.load(sys.stdin);print(len(d.get('events',[])))" 2>/dev/null)
echo "  OCSF: $OCSF_COUNT events"

# 7. Public read endpoint sanity
echo ""
echo "=== 7. Public read endpoint sanity ==="
PUBLIC_STATUS=$(curl -sk -o /dev/null -w '%{http_code}' "$BASE/api/range-public/showcase/atlas")
echo "  GET /api/range-public/showcase/atlas: $PUBLIC_STATUS (expect 200)"

# 8. Emit summary
echo ""
echo "=== 8. Writing summary to /tmp/atlas_summary.json ==="
echo "$META" > /tmp/atlas_meta.json
cat > /tmp/atlas_summary.json <<JSON
{
  "showcase": "atlas",
  "model_id": "$MODEL_ID",
  "isolation": {
...
JSON
echo "  wrote /tmp/atlas_meta.json (model meta)"
echo "  wrote /tmp/atlas_summary.json"
echo ""
echo "DONE."
