...
#
# After /api/range-form completes, this:
#   1. lists models for docsouth_showcase tenant
...
#
# Requires:  /tmp/doctoken (docsouth_showcase bearer)
# Output:    /tmp/docsouth_summary.json

set -e
BASE="https://www.latentocean.com"
DOCTOK=$(cat /tmp/doctoken)

# 1. List models
echo "=== 1. List models for docsouth_showcase tenant ==="
LIST=$(curl -sk -H "Authorization: Bearer $DOCTOK" "$BASE/api/range-form")
MODEL_ID=$(echo "$LIST" | python -c "import sys,json;d=json.load(sys.stdin);ms=d.get('models',[]);print([m for m in ms if 'DocSouth' in m['name']][0]['id'] if any('DocSouth' in m['name'] for m in ms) else (ms[0]['id'] if ms else ''))" 2>/dev/null)
if [ -z "$MODEL_ID" ]; then
  echo "  ERROR: no DocSouth model found"; exit 1
fi
...
META=$(curl -sk -H "Authorization: Bearer $DOCTOK" "$BASE/api/range-form/$MODEL_ID")
echo "$META" | python -c "
import sys, json
...
for intent in what_is_rare novel_classes show_taxonomy describe_corpus compare_to_llm summarize lineage_trace; do
  Q="${INTENTS[$intent]}"
  RESP=$(curl -sk -H "Authorization: Bearer $DOCTOK" "$BASE/api/range-query?model_id=$MODEL_ID&q=$(python -c "import urllib.parse,sys;print(urllib.parse.quote(sys.argv[1]))" "$Q")")
...
PROBE_TOK_RESP=$(curl -sk -X POST -H 'Content-Type: application/json' -d '{"color":"docsouthprobe"}' "$BASE/api/range-demo-token")
...
echo "  GET docsouth_showcase model with probe token: $PROBE_STATUS (expect 404)"
SELF_STATUS=$(curl -sk -o /dev/null -w '%{http_code}' -H "Authorization: Bearer $DOCTOK" "$BASE/api/range-form/$MODEL_ID")
echo "  GET docsouth_showcase model with own token:   $SELF_STATUS (expect 200)"

# 6. Pull audit
echo ""
echo "=== 6. Audit log ==="
JSON_AUDIT=$(curl -sk -H "Authorization: Bearer $DOCTOK" "$BASE/api/range-audit?limit=20")
...
CEF_LINES=$(curl -sk -H "Authorization: Bearer $DOCTOK" "$BASE/api/range-audit?format=cef&limit=20" | wc -l)
echo "  CEF: $CEF_LINES lines"
OCSF_COUNT=$(curl -sk -H "Authorization: Bearer $DOCTOK" "$BASE/api/range-audit?format=ocsf&limit=20" | python -c "import sys,json;d=json.load(sys.stdin);print(len(d.get('events',[])))" 2>/dev/null)
echo "  OCSF: $OCSF_COUNT events"

# 7. Emit summary
echo ""
echo "=== 7. Writing summary to /tmp/docsouth_summary.json ==="
python <<EOF
import json
with open('/tmp/_meta.json') as f: pass
EOF
echo "$META" > /tmp/_meta.json
echo "{\"model_id\":\"$MODEL_ID\",\"first_digests\":$(python -c "import json;import os;print(json.dumps({k.split('=')[0].strip():''}))"),\"isolation\":{\"probe_status\":$PROBE_STATUS,\"self_status\":$SELF_STATUS},\"audit\":{\"json\":$JSON_COUNT,\"cef\":$CEF_LINES,\"ocsf\":$OCSF_COUNT}}" > /tmp/docsouth_summary.json

cp /tmp/_meta.json /tmp/docsouth_meta.json
echo "  wrote /tmp/docsouth_meta.json (model meta)"
echo "  wrote /tmp/docsouth_summary.json"
echo ""
echo "DONE."
