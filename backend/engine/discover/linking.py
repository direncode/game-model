...
Ported from app.services.data_layer.linking. Changes from original:
  - Replaced ``from .types import CausalLink`` with inline dataclass
    (avoids pulling in the full data-layer types module).
...
Design notes:
  * Cosine runs on the *8D unit-sphere coordinates*, not the 384D raw
    ...
    in 8D is a reasonable proxy for 384D similarity.
  ...
  * All four signals scan the ``entity["attributes"]`` of the raw
    ...
    transparently json-loads that string before searching.
"""
...
def link_by_cosine(
    ...
    so we use a single matmul. O(n_a * n_b).
    """
...
def link_by_semantic_field(
    ...
    ``SEMANTIC_FIELDS`` and with strength 0.8.
    """
    index: dict[tuple[str, str], list[str]] = {}
    for s in survivors_a:
        attrs = _entity_attrs(s)
