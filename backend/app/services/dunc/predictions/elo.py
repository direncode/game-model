...
FIFA adopted ELO in 2018. Uses margin-of-victory multiplier.
"""
...
class FootballELO:
    ...
    def margin_multiplier(self, goal_diff: int) -> float:
        return max(np.log(abs(goal_diff) + 1) * (2.2 / 2.2), 1.0)
    ...
    def update(self, home: str, away: str, home_goals: int, away_goals: int) -> tuple[float, float]:
        ...
        self.ratings[away] = self.ratings.get(away, 1500.0) + self.k * m * (s_away - e_away)
    ...
    def compute_elo_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, row in df.iterrows():
            ...
            elo_features.append({
                ...
                "elo_expected_home": e_home, "elo_expected_away": 1 - e_home,
            })
            if pd.notna(row.get("FTHG")) and pd.notna(row.get("FTAG")):
                self.update(home, away, int(row["FTHG"]), int(row["FTAG"]))
        return pd.concat([df.reset_index(drop=True), pd.DataFrame(elo_features)], axis=1)
