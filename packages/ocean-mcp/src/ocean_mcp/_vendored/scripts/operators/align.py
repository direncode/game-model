...
# Stripped classes from this file: DispersionAnalysis
# ─────────────────────────────────────────────────────────
...
class ModuleAlignment(Operator):
    ...
    def run(self, inputs, *, seed, config):
        ...
        for m in modules:
            ...
            mod_aligned = dict(m)
            mod_aligned['alignment'] = {'k_nearest': k_eff, 'dominant_label': dom_label, 'dominant_share': round(dom_share, 3), 'bleed_share': round(bleed, 3), 'top3_labels': [{'label': l, 'count': c, 'share': round(c / k_eff, 3)} for l, c in top3], 'fine_label_dist': dict(Counter(nn_fines.tolist())), 'sample_records': [{'paper_id': str(p), 'label': str(l), 'title': str(t)} for p, l, t in zip(nn_ids[:5], nn_labels[:5], nn_titles[:5])]}
